...
$sql_online   = "SELECT COUNT(*) as total FROM visitor_useronline";
$count_online = $connect->query($sql_online)->fetch(PDO::FETCH_OBJ)->total;

// 3. Total Vistas de Páginas (Tabla visitor_pages)
$sql_views   = "SELECT SUM(visitor_pages_total_views) as total FROM visitor_pages";
$total_views = $connect->query($sql_views)->fetch(PDO::FETCH_OBJ)->total;
...
$sql_top_pages = "SELECT visitor_pages_title, visitor_pages_uri, visitor_pages_total_views, visitor_pages_type 
                  FROM visitor_pages 
                  ORDER BY visitor_pages_total_views DESC LIMIT 5";
