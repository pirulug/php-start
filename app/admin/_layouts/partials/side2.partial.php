  ...
  <div class="sidebar-content js-simplebar">
    ...
    <ul class="sidebar-nav">
      <?php foreach ($routes as $key => $item): ?>
        ...
        if (empty($item['icon']) || !$accessManager->can_access($item['path'])) continue;
        ?>
        ...
        <?php if (!empty($item['collapsed'])): ?>
          ...
          <li class="sidebar-item <?= $active ? 'active' : '' ?>">
            ...
            <ul class="sidebar-dropdown list-unstyled collapse <?= $active ? 'show' : '' ?>"
                id="<?= $item['path'] ?>" data-bs-parent="#sidebar">
              <?php foreach ($item['items'] as $link => $subItem): ?>
                <?php
                // Saltar los items ocultos
                if (!empty($subItem['hidden'])) continue;
                if (!$accessManager->can_access($subItem['path'])) continue;
                ?>
