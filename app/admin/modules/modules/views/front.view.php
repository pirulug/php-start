...
<form action="<?= admin_route("modules/front") ?>" method="POST" id="modules-form">
  <div class="card mb-3">
    ...
    <div class="card-body p-3">
      <div class="mb-3 border-bottom pb-2">
        <h5 class="card-title mb-1">Módulos de Frontend</h5>
        <p class="text-muted small mb-0">Habilita o deshabilita los módulos disponibles en el sitio web público.</p>
      </div>
      
      <div class="list-group module-list-group">
        <?php foreach ($view_front_modules as $name => $info): ?>
          <div class="list-group-item d-flex align-items-center justify-content-between p-3">
            <div>
              ...
              <span class="text-muted small">Ubicado en /app/front/modules/<?= clear_html($name) ?></span>
            </div>
            
            <div class="form-check form-switch m-0 d-flex align-items-center gap-2">
              <input class="form-check-input" type="checkbox" role="switch" name="front_active[<?= clear_html($name) ?>]" id="switch_front_<?= clear_html($name) ?>" <?= $info["active"] ? "checked" : "" ?> <?= $info["protected"] ? "disabled" : "" ?>>
