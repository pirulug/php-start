...
if (!$user) {
  header("Location: " . admin_route("sign-out"));
  exit();
}
...
if ($is_2fa_enabled) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['disable_2fa'])) {
    ...
    // Validar contraseña
    if ($current_password === '') {
      $notifier->danger("Debes ingresar tu contraseña para desactivar el 2FA.")
        ->bootstrap()
        ->add();
    } elseif (!$cipher->verifyPassword($current_password, $user->user_password)) {
      $notifier->danger("La contraseña actual ingresada es incorrecta.")
        ->bootstrap()
        ->add();
    }
    ...
    if (!$notifier->can()->danger()) {
      ...
      // Guardar log
      $log->info("Usuario {$user->user_login} desactivó la autenticación en dos factores (2FA)")
        ->file("dashboard")
        ->with("user_id", $user->user_id)
        ->write();

      $notifier->success("La autenticación de doble factor (2FA) ha sido desactivada correctamente.")
        ->bootstrap()
        ->add();

      header("Location: " . admin_route("account/settings/2fa"));
      exit();
    }
  }
} else {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['enable_2fa'])) {
    ...
    if (strlen($code) === 6 && $ga->verifyCode($temp_secret, $code)) {
      ...
      // Guardar log
      $log->info("Usuario {$user->user_login} activó la autenticación en dos factores (2FA)")
        ->file("dashboard")
        ->with("user_id", $user->user_id)
        ->write();

      $notifier->success("¡La autenticación de doble factor (2FA) ha sido activada y configurada con éxito!")
        ->bootstrap()
        ->add();
      ...
      $notifier->danger("El código ingresado es incorrecto o ha expirado. Por favor, intenta de nuevo.")
        ->bootstrap()
        ->add();
    }
  }
