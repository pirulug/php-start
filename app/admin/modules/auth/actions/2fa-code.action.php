...
// -----------------------------------------------------------------------------
// SECCIÓN: VERIFICACIÓN DE CÓDIGO 2FA (CODE)
// -----------------------------------------------------------------------------

if (!isset($_SESSION['2fa_user_id'])) {
  $notifier->danger("Acceso no autorizado. Por favor inicia sesión.")
    ->bootstrap()
    ->add();
...
if (!$meta || empty($meta->usermeta_value)) {
  ...
  $notifier->danger("Error de seguridad 2FA. Por favor inicia sesión de nuevo.")
    ->bootstrap()
    ->add();
  header("Location: " . admin_route("sign-in"));
  exit();
...
if (!$user) {
  unset($_SESSION['2fa_user_id']);
  $notifier->danger("Usuario no encontrado. Por favor inicia sesión de nuevo.")
    ->bootstrap()
    ->add();
  header("Location: " . admin_route("sign-in"));
  exit();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (strlen($code) === 6 && $ga->verifyCode($secret, $code)) {
    ...
    $log->info("Usuario {$user->user_login} inició sesión correctamente tras verificar 2FA")
      ->file("dashboard")
