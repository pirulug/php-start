...
<div class="container py-3">
  <div class="row justify-content-center">
    <div class="col-lg-9">
      ...
      <!-- HEADER PREMIUM -->
      <div class="card mb-3 overflow-hidden" style="background: linear-gradient(135deg, var(--pr-primary) 0%, #6610f2 100%); border: none;">
        <div class="card-body p-3 p-md-4 text-center">
          <div class="d-inline-flex align-items-center justify-content-center bg-white bg-opacity-25 rounded-circle mb-3" style="width: 60px; height: 60px;">
             <i class="fa-solid <?= $is_faq ? 'fa-circle-question' : 'fa-file-shield' ?> text-white fs-2"></i>
          </div>
          <h1 class="fw-bold text-white mb-1 text-uppercase small" style="letter-spacing: 2px;"><?= $is_faq ? 'Preguntas Frecuentes' : 'Legal / Políticas' ?></h1>
      ...
      <div class="card mb-3">
        <div class="card-body p-3 p-md-4">
          ...
          <div class="d-flex align-items-center gap-2 text-body-secondary small mb-4 pb-3 border-bottom border-opacity-10">
            <i class="fa-regular fa-clock"></i>
            <span class="text-uppercase fw-bold" style="font-size: 0.7rem; letter-spacing: 0.5px;">Última actualización: <?= site_date($policy->post_updated_at, 'date') ?></span>
          </div>
                ...
                <?php foreach ($faqs as $i => $f): ?>
                  <div class="accordion-item bg-transparent border-bottom border-opacity-10 py-2">
                    <h2 class="accordion-header">
                      <button class="accordion-button <?= $i === 0 ? '' : 'collapsed' ?> bg-transparent fw-bold shadow-none text-body" type="button" data-bs-toggle="collapse" data-bs-target="#faq-<?= $i ?>">
...
<style>
  ...
  .richtext-render p { margin-bottom: 1.25rem; }
  .richtext-render ul, .richtext-render ol { margin-bottom: 1.5rem; padding-left: 1.25rem; }
  .richtext-render li { margin-bottom: 0.5rem; }
