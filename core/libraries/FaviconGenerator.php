 ...
 * FaviconGenerator
 *
 * Generador de favicons y archivos de iconos multiplataforma a partir de una imagen PNG.
 * Crea todos los tamaños y formatos requeridos para navegadores web modernos y dispositivos móviles,
 * incluyendo iconos de Android, Apple, Windows y formato .ico. Utiliza un sistema de hashing
...
class FaviconGenerator {
  ...
  /**
   * Inicializa el generador definiendo el directorio de salida.
   *
   * @param string $uploadDir Ruta física para guardar los iconos.
   */
  ...
  // --------------------------------------------------------------------------
  // SECCIÓN: PROCESAMIENTO PRINCIPAL
  // --------------------------------------------------------------------------

  /**
   * Genera todo el set de iconos a partir de una imagen origen.
   * Requiere que el archivo sea un PNG válido.
   *
   * @param string $sourceFile Ruta física del archivo PNG origen.
   * @return array Listado de archivos generados mapeados por tipo.
   * @throws Exception Si la imagen no es válida o hay errores de procesamiento.
   */
  public function generate(string $sourceFile): array {
    ...
    $this->hash = substr(md5(uniqid(mt_rand(), true)), 0, 8);
  ...
  // --------------------------------------------------------------------------
  // SECCIÓN: LÓGICA INTERNA DE IMAGEN
  // --------------------------------------------------------------------------

  /**
   * Redimensiona y guarda una versión del icono en formato PNG.
   *
   * @param GdImage $sourceImage Imagen original.
   * @param int $width Ancho destino.
   * @param int $height Alto destino.
   * @param string $baseName Nombre base del archivo (sin hash ni extensión).
   */
  ...
  /**
   * Genera el archivo favicon.ico conteniendo múltiples tamaños mediante Imagick.
   *
   * @param GdImage $sourceImage Imagen original.
   */
  ...
  /**
   * Genera el archivo manifest.webmanifest para compatibilidad con PWA y móviles.
   */
